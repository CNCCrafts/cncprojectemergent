...
import { X, Send, Sparkles, ArrowUpRight, ImagePlus, Link2 } from 'lucide-react';
...
export default function CustomOrderModal({ open, onClose }) {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', category: 'acrylic', material: '',
    dimensions: '', quantity: 1, budget: '', timeline: '',
    message: '', imageUrl: '',
  });
  ...
  const materials = MATERIALS_BY_CATEGORY[form.category] || [];
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl('/api/custom-order'), {
        method: 'POST',
        body: fd,
      });
            ...
            <form onSubmit={submit} data-testid="custom-order-form">
              ...
              {materials.length > 0 && (
                <div className="form-group">
                  ...
                  <select value={form.material} onChange={set('material')} data-testid="co-material">
                    <option value="">Select a material…</option>
                    {materials.map(m => <option key={m.id} value={m.label}>{m.label}</option>)}
                  </select>
                </div>
              )}
              ...
              {/* Image upload + URL */}
              <div className="form-group">
                <label>Design Reference (image or URL)</label>
                <div className="co-image-upload" data-testid="co-image-upload">
                  <input type="file" accept="image/*" onChange={handleFile} id="co-image-file" data-testid="co-image-file" hidden />
                  <button
                    type="button"
                    className="co-image-btn"
                    onClick={() => document.getElementById('co-image-file')?.click()}
                  >
                  ...
                  {preview ? (
                    <div className="co-image-preview">
                      ...
                      <button type="button" onClick={() => { setImageFile(null); setPreview(''); }} aria-label="Remove image">×</button>
                    </div>
                  ) : (
