...
import { ShoppingBag, Search, Check, PenLine } from 'lucide-react';
...
const SECTIONS = [
  ...
  { id: 'gifting', label: 'Gifting Picks', kicker: '06', icon: 'ti-gift',           desc: 'Thoughtful personalised pieces made to delight on birthdays, anniversaries and special occasions.' },
  { id: 'custom',  label: 'Custom Order',  kicker: '07', icon: 'ti-pencil-plus',    desc: 'Have a unique vision? Get a personalised quote from our craft team.' },
];

function ProductCard({ product, onAdd, onCustom, onOpen }) {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onOpen(product.id)}
    >
...
{isCustom ? (
            <button
              ...
              data-testid={`custom-quote-${product.id}`}
            >
  ...
  return (
    <main className="categories-page">
      ...
      <div className="gifting-hero">
        <div className="container">
          <div className="gifting-hero__content">
            <span className="eyebrow eyebrow--center" style={{ color: '#EEC7B4', marginBottom: 12 }}>Thoughtful gifting</span>
            <h2 className="gifting-hero__title">Beautiful keepsakes for every celebration.</h2>
            <p className="gifting-hero__text">From desk-worthy pen stands to personalized decor, our gifting range brings warmth, elegance and meaning to every special moment.</p>
          </div>
        </div>
      </div>
